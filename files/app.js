...
app.controller('MainCtrl', ['$scope', '$q', 'pingCheckService',
    function ($scope, $q, $ping) {
        $scope.iptable = [];
        $scope.maxPing = 300;
        $scope.count = 0;
        $scope.allcount = 0;
        ...
        $scope.pingFilter = function (item) {
            if (item.ok)
                return true;
            if ($scope.count / $scope.allcount >= 0.9 && $scope.okcount < $scope.maxcount)
                return true;
            if (isNaN(item.ping))
                return false;
        };
        ...
        $scope.begin = function (maxPing) {
            if ('undefined' == typeof (maxPing))
                $ping.maxPing = $scope.maxPing / 1.5;
            else
                $ping.maxPing = maxPing / 1.5;
...
]).service('pingCheckService', ['$http', '$q', '$timeout',
